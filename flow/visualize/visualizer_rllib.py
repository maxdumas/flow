...
def visualizer_rllib(args):
    ...
    if args.run and config_run:
        if args.run != config_run:
            ...
            sys.exit(1)
    if args.run and args.run == "PPO":
        agent_cls = PPOTrainer
    elif config_run and config_run == "PPO":
        agent_cls = PPOTrainer
    else:
        print(
            "visualizer_rllib.py: error: could not find flow parameter "
            "'run' in params.json, "
    ...
    if multiagent:
        for agent_id, rew in rets.items():
            ...
            print(rew)
            print(
                "Average, std return: {}, {} for agent {}".format(
    ...
    print("\nSpeed, mean (m/s):")
    print(mean_speed)
    print("Average, std: {}, {}".format(np.mean(mean_speed), np.std(mean_speed)))
    print("\nSpeed, std (m/s):")
    print(std_speed)
...
def create_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="sumo_gui",
        help="Pick the render mode. Options include sumo_web3d, " "rgbd and sumo_gui",
    )
    parser.add_argument(
        "--save_render",
        action="store_true",
