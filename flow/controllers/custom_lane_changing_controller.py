...
class LaneChangeController_AvoidAVClumping(BaseLaneChangeController):
    ...
    def get_lane_change_action(self, env):
        # """See parent class."""
        # print(self.veh_id, env.time_counter, env.k.vehicle.get_route(self.veh_id))
...
class LaneChangeController_AvoidAVClumping_OffRamps(BaseLaneChangeController):
    """A controller used to enforce sumo lane-change dynamics on a vehicle.
    ...
    def get_lane_change_action(self, env):
        ...
        if h <= HEADWAY_ALERT and (self.leader_more_than_two_av(env, rl_ids, lead_id) or self.between_two_av(env)):
            ...
            # if the car wants to exit highway, the direction can only be 0 or 1
            if self.is_exit_highway(env):
                if current_lane == 0:
                    # Already at the right-most lane
                    change_direction = 0
                elif change_direction == 1:
                    lane_choices = np.sign(np.array(available_lanes) - current_lane)
                    if -1 in lane_choices:
                        change_direction = -1
                    else:
                        change_direction = 0
    ...
    def is_exit_highway(self, env):
        routes = env.k.vehicle.get_route(self.veh_id)
        for i in range(len(routes)- 1, -1, -1):
            if routes[i].startswith('off_ramp'):
                return True
        return False
